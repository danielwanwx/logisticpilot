...
Model candidates are deliberately blocked unless `--execute-model` is supplied. Nova Pro development is stopped; its private v5–v7 records remain part of the durable experiment ledger. The v8 candidate uses only Claude Opus 4.6 (`us.anthropic.claude-opus-4-6-v1`, `us-west-2`, profile `missing20-sandbox`, temperature 0), sets `retry_strategy=None`, and creates its Bedrock client with `total_max_attempts: 1`. There is no fallback or general model routing. This is a new model configuration, not evidence of an architectural gain; the business schemas, read tools, prompts, evaluator, and Graph are unchanged. The model adapter lowers and verifies the delegate's `max_tokens` before every request, then reserves the same limit. It records unknown usage as the full reservation. Opus pricing is USD 0.0000055 per input token and USD 0.0000275 per output token. The per-workflow limit is eight provider requests, 32,000 input tokens, 6,000 output tokens, USD 0.35, and 180 seconds; those maximum token limits estimate to USD 0.341. The fixed Graph splits this into 3/3/2 requests with 12k/12k/8k input and 2k output per node; its native `Limits` are supplementary only. See the [v8 Opus addendum](../../docs/research/2026-09-12-opus-paired-evaluation-addendum.md).
